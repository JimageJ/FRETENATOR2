...
def previewDialog(imp, options):
	...
	segmentChannel, donorChannel, acceptorChannel, acceptorChannel2, nearbyQuantCh, thresholdMethod, maxIntensity, gaussianSigma, largeDoGSigma, DoG,  manualSegment, manualThreshold, makeNearProj, dilation, sizeExclude, minSize, maxSize, watershed, backsubVal, pixelByPixel, saveSettings =options
	...
	if (segmentChannel<= len(types) and
		...
		gd.addChoice("Nearby quant channel", types, str(nearbyQuantCh))
	else:
		if len(types)>2:
			...
			gd.addChoice("Nearby quant channel", types, types[2])
			#print('YAY')
		else:
			...
			gd.addChoice("Nearby quant channel", types, types[-1])
	...
	gd.addSlider("Manual threshold", 4, 65534, manualThreshold, 1)
	...
	Rowe, JH., et al. (2023) Next-generation ABACUS biosensors reveal cellular
	...
	nearbyQuantCh=int(choices.get(4).getSelectedItem())
	thresholdMethod=choices.get(5).getSelectedItem()
	dilation=int(choices.get(6).getSelectedItem())
	maxIntensity=int(choices.get(7).getSelectedItem())
	backSub= choices.get(8).getSelectedItem()
	...
	gfx1,gfx2,gfx3,gfx4,gfx5 = segment(gfx1,gfx2,gfx3,gfx4,gfx5, gaussianSigma, thresholdMethod, maxIntensity, largeDoGSigma,
	pixelAspect, originalTitle, DoG,  manualSegment, manualThreshold, dilation, sizeExclude, minSize, maxSize, watershed)
	...
	if backSub=="Global mean":
		backsubVal=2
	return segmentChannel, donorChannel, acceptorChannel, acceptorChannel2, nearbyQuantCh,  thresholdMethod, maxIntensity, gaussianSigma, largeDoGSigma, DoG,  manualSegment, manualThreshold, makeNearProj, dilation, sizeExclude, minSize, maxSize, watershed, backsubVal, pixelByPixel, saveSettings
...
def fretCalculations(imp1, nFrame, donorChannel, acceptorChannel, acceptorChannel2,nearbyQuantCh, table, gfx1, gfx2, gfx3, gfx4, gfx5, originalTitle, backSub, pixelByPixel):
	...
	nearbyQuantImp=extractChannel(imp1, nearbyQuantCh, nFrame)
	...
	if backSub==1:
		...
		clij2.dilateLabels(gfx1, gfx7 , 4)
	...
	if backSub==1:
		results4=ResultsTable()
		gfx8=clij2.push(nearbyQuantImp)
		clij2.statisticsOfBackgroundAndLabelledPixels(gfx8, gfx7, results4)
	...
	for i in xrange(len(acceptorChIntensity)):
		if (acceptorChIntensity[i]>0) and (donorChIntensity[i]>0):
			...
			if backSub==1:
				table.addValue("Nearby mCherry mean", results4.getValue("MEAN_INTENSITY",i))
		else:
...
if exists('FRETENATOR2SegSettingsBT.pckl'):
	print('exists')
	sf=file('FRETENATOR2SegSettingsBT.pckl', 'rb')
	options=pickle.load(sf)
	sf.close()
	print(options)
else:
	print('Default settings')
	options=(3, 1, 2, 3, 5, 'Otsu', 4094, 0.8, 4.0, True, False, 3000, True, 0, False, 10, 10000, True, 0, False, True)
...
IJ.log("segmentChannel, donorChannel, acceptorChannel, acceptorChannel2, nearbyQuantCh, thresholdMethod, maxIntensity, gaussianSigma, largeDoGSigma, DoG, manualSegment, manualThreshold, makeNearProj, dilation, sizeExclude, minSize, maxSize, watershed, backSub:")
IJ.log(str(options))

segmentChannel, donorChannel, acceptorChannel, acceptorChannel2, nearbyQuantCh, thresholdMethod, maxIntensity, gaussianSigma, largeDoGSigma, DoG,  manualSegment, manualThreshold, makeNearProj, dilation, sizeExclude, minSize, maxSize, watershed, backSub, pixelByPixel, saveSettings =options
if saveSettings==1:
	sf=file('FRETENATOR2SegSettingsBT.pckl', 'wb')
...
for nFrame in xrange(1, totalFrames):
	...
	table, FRETimp2, FRETProjImp, labelImp=fretCalculations(imp1, nFrame, donorChannel, acceptorChannel, acceptorChannel2,nearbyQuantCh, table, gfx5, gfx2, gfx3, gfx4, gfx1, originalTitle,backSub, pixelByPixel)
...
IJ.setMinAndMax(conFRETImp2, 1000, 5500)
conFRETImp2.show()
IJ.run("000_Turbo")
...
IJ.setMinAndMax(conFRETProjImp, 1000, 5500)
...
if makeNearProj == True:
	...
	IJ.setMinAndMax(nearZImpOutlines, 1000, 5500)
